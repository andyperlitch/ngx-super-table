...
const GLOBALS = {
  ...
  'lodash-es/forEach': '_.forEach',
  'lodash-es/values': '_.values',
  'rxjs': 'Rx',
...
function createUmd(globals) {
  const name = 'ngx-super-table';
  const entry = `${process.cwd()}/dist/es5/index.js`;
  return generateBundle(
    entry,
    `${process.cwd()}/dist/packages-dist/ngx-super-table.umd.js`,
    globals,
    name,
    'umd',
  );
...
function createEs(globals, target) {
  ...
  return generateBundle(
    entry,
    `${process.cwd()}/dist/packages-dist/ngx-super-table.${target}.js`,
    globals,
    name,
    'es',
  );
...
function copyFiles() {
  ...
  return Observable
    .forkJoin(
      ...
      Observable.from(copy(`${process.cwd()}/MIT-LICENSE`, `${process.cwd()}/dist/packages-dist/MIT-LICENSE`)),
    );
